...
# filename = 'DATA/folderB/dataB.dat'
# column_number = 3
#
...
#
# autoper = Autoperiod(np.linspace(0, len(float_array), len(float_array)), float_array)
# print(autoper.period())
...
while True:
    ...
    for i in range(4):
        ...
        with open(filename, 'r') as file:
            ...
            for k in range(len(string_array)):
                for j in range(len(string_array[k].split(' ')) - 1):
                    if j == i + 1:
                        float_array.append(float(string_array[k].split(' ')[j]))

        autoper = Autoperiod(np.linspace(0, len(float_array), len(float_array)), float_array)
        value_lst.append(autoper.period)

    for i in range(4):
        filename = 'DATA/folderF/dataF.dat'
    ...
    if not value_lst.__contains__(None):
        break

    value_lst.clear()

result = np.array(value_lst)
print(result.mean())
print("Stop")
